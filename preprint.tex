...
Note that when \(s=0\), \(\mathbf{Tr}(\mathbf{M}) = 0\) (the classifier
is \emph{always} wrong), when \(s=0.5\), the classifier is no-skill and
...
above, we can do so by defining a bias matrix \# Numerical experiments
